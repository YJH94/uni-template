<template>
	<view>
		...
		<picker-view @change="dateTimePickerChange" @pickstart="start" @pickend="end" :value="selectDateTime">
...
export default {
	...
	created() {
		...
		if (this.maxDateStr) {
			this.setMaxDate(this.maxDateStr);
		}
		if (this.value) {
	...
	methods: {
		...
		setDefaultValue(val) {
			...
			if (val) {
				try {
					...
					if (this.maxDateStr) {
						console.log(this.maxDateStr);
						this.setDateWithMax(defaultYear,defaultMon)
					}
					setTimeout(() => {
						//渲染完才生效
						if (!this.isTime) {
		...
		setMaxDate(value){
			if (value) {
				...
				this.maxDate = this.$DateUtils.dateFromString(dateStr);
